...
<div>
    ...
    <div class="recommend" ref='wrapper'>
        ...
        <div class="items">
            <div v-for="item in songList" class="item-box" :key='item.id'>
                    <div class="icon">
                        <i class="iconfont icon-icon-test"><span class="test-num">{{(item.accessnum/10000).toFixed(1)}}万</span></i>
                        <i class="iconfont icon-shuangsechangyongtubiao-"></i>
                    </div>
                    <img :src="item.picUrl" alt="" class="item-imag" >
                    <div class="song-message"><i class="iconfont icon-yaowan1"></i><div>{{item.songListDesc}}</div></div>
            </div>
            <div class="get-more">...获取更多<i class="iconfont icon-jiantouzuo"></i></div>
        </div>
        </div>
    </div>
</div>
...
@import '~stylus/variable.styl'
    .line
        width 0rem
        margin-top 1rem
        border-bottom .1rem solid  $color-them-icon
